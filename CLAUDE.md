...
**Data ingestion** (`update:data` at `:05`/`:35` in `routes/console.php` → `fetch:metars`, `fetch:tafs`, `fetch:vatsim`, `calc:scores`; `fetch:bookings` separately every 30 min): METARs and TAFs come from the aviationweather.gov bulk cache files (`{metars,tafs}.cache.xml.gz` — download, gunzip, parse, delete; see `AviationWeatherHelper`), never per-airport API calls. `tafs` stores one row per forecast period with structured fields (no regex re-parsing — `Taf` mirrors `Metar`'s condition-method names over those fields). `bookings` stores the next 24h of VATSIM ATC bookings resolved to airports via `AirportCallsignHelper::resolveIcao()` (shared with `fetch:vatsim`; unresolvable/CTR/FSS/ATIS positions are dropped, never stored with a null FK).
...
**ETA matching**: score lookups are time-scoped. `AirportScore::coversEta()` (SQL) is the single definition — exact window containment for `metar`/`taf`/`vatsim`/`booking`, ±2h overlap for `event`/`logon_estimate`, and a METAR-fallback branch (a `metar` row matches regardless of window when no scoreable TAF period covers the ETA). It has a PHP twin, `coversEtaAt()`/`Airport::scoresAtEta()`, used to filter loaded collections per candidate — **keep the two in sync when changing matching rules**. Search binds a per-candidate ETA computed inside SQL (`CalculationHelper::forecastEtaSql()`, distance-derived); "now" views pass `now()`. Rankings count `COUNT(DISTINCT reason)` (several sources can assert the same reason); rendering dedupes via `Airport::displayScores()`.
