        ...
        foreach($airports as $airport){
            ...
            foreach($airport->runways as $runway){
                if(!$runway->closed){
                    ...
                    // Check crosswind at runways
                    if(!empty($airport->metar->wind_direction) && !empty($runway->le_heading) && !empty($runway->he_heading)){
                        if($airport->metar->wind_speed >= 15){

                            $runwayCrosswindComponent = abs($airport->metar->wind_speed * sin(($airport->metar->wind_direction - $runway->le_heading)/180*pi()));
                            if($runwayCrosswindComponent >= 12){
